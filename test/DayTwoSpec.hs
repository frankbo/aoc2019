...
spec = describe "DayTwo" $ do
  ...
  it "should add and the stop" $ do
    (intCodeProgram [1, 0, 0, 0, 99, 1, 2, 3])
      `shouldBe` [2, 0, 0, 0, 99, 1, 2, 3]
  it "should first add then muliply" $ do
    (intCodeProgram [1, 0, 0, 0, 2, 0, 0, 3, 99])
      `shouldBe` [2, 0, 0, 4, 2, 0, 0, 3, 99]
  ...
  it "should work with the example two" $ do
    (intCodeProgram [1, 1, 1, 4, 99, 5, 6, 0, 99])
