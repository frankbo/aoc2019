...
intCodeProgram input =
  ...
  in  do
        noun <- [0 .. 99]
        verb <- [0 .. 99]
        True <- return (V.elem 19690720 (findVerbAndNoun verb noun list))
...
computer list currentPosition =
  let opcode      = V.take 4 (V.drop currentPosition list)
  ...
  in  if currentPosition <= V.length list && instruction /= 99
        then case instruction of
          1 -> computer (add list idx first second) (currentPosition + 4)
          2 -> computer (multiply list idx first second) (currentPosition + 4)
